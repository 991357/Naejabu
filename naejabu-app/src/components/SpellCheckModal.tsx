...
const SpellCheckModal: React.FC<SpellCheckModalProps> = ({ isOpen, onClose, text, results, onCorrectAll }) => {
  ...
  const renderHighlightedText = () => {
    ...
    results.forEach((result, i) => {
        ...
        parts.push(
            <span key={i} className={`px-1 rounded ${selectedError?.token === token ? 'bg-yellow-200' : 'bg-red-200'}`}>
                {token}
            </span>
        );
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl h-4/5 flex flex-col">
        <div className="p-6 border-b">
          <h2 className="text-2xl font-bold">맞춤법 검사</h2>
        </div>
        <div className="flex-grow flex p-6 overflow-hidden">
          ...
          <div className="w-1/2 pl-6 border-l overflow-y-auto">
            <h3 className="text-xl font-semibold mb-4">수정 제안</h3>
            <div className="space-y-4">
              {results.map((result, i) => (
                <div 
                    key={i} 
                    className={`p-4 border rounded-lg cursor-pointer ${selectedError?.token === result.token ? 'border-blue-500' : ''}`}
                    onClick={() => setSelectedError(result)}
                >
                  <p className="text-red-500 font-semibold">{result.token}</p>
                  <p className="text-sm text-gray-600 mb-2">{result.info}</p>
                  {result.suggestions.map((suggestion: string, j: number) => (
                    <button
                      key={j}
                      onClick={() => handleCorrection(result.token, suggestion)}
                      className="bg-blue-100 hover:bg-blue-200 text-blue-800 text-sm font-semibold py-1 px-3 rounded-full mr-2 mb-2"
                    >
                      {suggestion}
                    </button>
                  ))}
        ...
        <div className="p-6 border-t flex justify-end">
          <button onClick={onClose} className="mr-4 px-6 py-2 rounded bg-gray-200 hover:bg-gray-300">
            취소
          </button>
          <button onClick={() => onCorrectAll(correctedText)} className="px-6 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">
            수정 완료
          </button>
        </div>
      </div>
