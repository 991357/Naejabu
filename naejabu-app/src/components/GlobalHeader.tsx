...
const GlobalHeader = () => {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      router.push('/');
      // Force a reload to re-trigger the auth check
  ...
  if (loading) {
    return (
        <header className="w-full h-16 bg-white shadow-md"></header>
    );
  }
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`text-lg font-semibold transition-colors duration-300 ${pathname.startsWith(link.href) && link.href !== '#' ? 'text-primary' : 'text-gray-600 hover:text-primary'}`}>
                  {link.label}
                </Link>
              ))}
            </div>
